...
pub fn record(device: Option<&i32>, output: Option<&PathBuf>, lib: Option<&PathBuf>, silence_threshold_ms: Option<&u64>) {
    let mut recorder_builder = create_recorder_builder(lib);
    ...
    let mut wav_writer = if let Some(output_file) = output {
        Some(create_wav_writer(output_file, sample_rate))
    } else {
        None
    };
            ...
            Ok(frame) => {
                ...
                if is_silence {
                    ...
                    if let Some(threshold) = silence_threshold_ms {
                        if silence_duration_ms >= *threshold {
...
fn create_recorder_builder(lib: Option<&PathBuf>) -> PvRecorderBuilder {
    let mut recorder_builder = PvRecorderBuilder::new(512);

    let library_path = match lib {
...
fn create_wav_writer(output_path: &PathBuf, sample_rate: u32) -> WavWriter<File> {
